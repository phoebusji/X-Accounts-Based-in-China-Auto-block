...
2. 将脚本控制面板中的选项"使用本地Blocked缓存"去掉.
3. 进入Blocked Accounts页面(https://x.com/settings/blocked/all).
4. 手动向下翻页，刷出一些数量的blocked账号。
5. 滚动条拖到最上面，运行脚本，脚本自动翻页向下刷出所有账号。
6. 在自动获取blocked的时候，脚本运行时需要处于前台，不能切换标签页，不能锁屏，等待完全获取后，可以切换到后台。
7. 也可以手动刷出所有账号后，再运行脚本。
8. 可能由于twitter的限制，有时刷了一会刷不出来了，可以手动往回翻几页再往后翻页。
9. 由于每次获取blocked的脚本非常慢，添加了选项("使用本地Blocked缓存")从上次获取的blocked的本地存储来运行来节省时间，但注意需要定期完整网页获取。首次运行需要完整网页获取。
10. 由于twitter 的短时间blocked数量限制，会出现401错误，账号被强制退出，需要重新登录账号。
11. 我自己运行的感觉twitter每天block的上限好像是1000左右。
12. 脚本中的2个地方可以根据自己情况修改：
